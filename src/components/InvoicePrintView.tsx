...
export default function InvoicePrintView({ invoice }: { invoice: Invoice }) {
  ...
  const hasBankDetails = Boolean(invoice.bank_name || invoice.iban || invoice.swift);
        ...
        {hasBankDetails && (
          <div className="mb-8 rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs text-gray-700">
              {invoice.bank_name && <p><span className="font-semibold text-gray-900">Banca:</span> {invoice.bank_name}</p>}
              {invoice.iban && <p><span className="font-semibold text-gray-900">IBAN:</span> {invoice.iban}</p>}
              {invoice.swift && <p><span className="font-semibold text-gray-900">SWIFT:</span> {invoice.swift}</p>}
            </div>
          </div>
        )}
