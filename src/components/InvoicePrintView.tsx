  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6 no-print">
        ...
        <button onClick={() => window.open(`/print/${invoice.id}`, "_blank")} className="ml-auto flex items-center gap-2 bg-gray-900 text-white rounded-lg px-4 py-2 text-sm hover:bg-gray-800 transition-colors">
          <Printer size={15} /> Printează / PDF
        </button>
      </div>

      <div className="bg-white shadow-sm rounded-xl border border-gray-200 max-w-3xl mx-auto p-10">
        <p className="text-[9px] text-gray-400 mb-6 leading-relaxed">Factura circula fara semnatura si stampila conform legislatiei in vigoare.</p>
            ...
            <div className="text-xs text-gray-600 space-y-0.5">
              ...
              {(invoice.bank_name || invoice.iban || invoice.swift) && (
                <div className="mt-2 pt-2 border-t border-gray-200 space-y-0.5">
                  {invoice.bank_name && <p>Banca: {invoice.bank_name}</p>}
        ...
        <table className="w-full mb-6 text-sm">
          <thead><tr className="border-b-2 border-gray-300"><th className="text-left pb-2 text-xs font-semibold text-gray-500 uppercase">#</th><th className="text-left pb-2 text-xs font-semibold text-gray-500 uppercase">Articol</th><th className="text-center pb-2 text-xs font-semibold text-gray-500 uppercase">U.M.</th><th className="text-center pb-2 text-xs font-semibold text-gray-500 uppercase">Cant.</th><th className="text-right pb-2 text-xs font-semibold text-gray-500 uppercase">Pret unitar</th><th className="text-right pb-2 text-xs font-semibold text-gray-500 uppercase">Valoare</th><th className="text-right pb-2 text-xs font-semibold text-gray-500 uppercase">TOTAL</th></tr></thead>
          <tbody>{invoice.items.map((item, index) => <tr key={index} className="border-b border-gray-100"><td className="py-2 pr-3 text-gray-400">{index + 1}</td><td className="py-2 pr-3">{item.description}</td><td className="py-2 text-center text-gray-600">{item.unit}</td><td className="py-2 text-center">{Number(item.quantity)}</td><td className="py-2 text-right">{fmt(Number(item.unit_price))}</td><td className="py-2 text-right">{fmt(Number(item.total))}</td><td className="py-2 text-right font-medium">{fmt(Number(item.total))}</td></tr>)}</tbody>
        </table>

        <div className="flex justify-end mb-6"><div className="w-64"><div className="flex justify-between py-1.5 text-sm text-gray-600"><span>Total fara TVA:</span><span className="font-medium">{fmt(total)} {invoice.currency}</span></div><div className="flex justify-between py-2 bg-blue-600 text-white px-3 rounded-sm mt-1"><span className="font-semibold">Total</span><span className="font-bold">{fmt(total)} {invoice.currency}</span></div></div></div>
        {totalRON && invoice.exchange_rate && <p className="text-xs text-gray-500 mb-6">Valori in LEI (curs: {invoice.exchange_rate}): VALOARE {fmt(totalRON)} RON, TVA 0 RON, TOTAL {fmt(totalRON)} RON</p>}
        {invoice.notes && <p className="text-xs text-gray-600 mt-4 italic">{invoice.notes}</p>}
        <div className="mt-12 pt-4 border-t border-gray-100 text-[10px] text-gray-400">{invoice.invoice_number} {fmt(total)} {invoice.currency} scadenta la {format(new Date(invoice.due_date), "dd.MM.yyyy")}</div>
      </div>
    </div>
  );
}
